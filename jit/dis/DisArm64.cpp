...
#include "jit/Common/Arm64Emitter.h"
...
static void DataProcessingImmediate(uint32_t w, uint64_t addr, Instruction *instr) {
	...
	if (((w >> 23) & 0x3f) == 0x25) {
		...
		if (op & 1) imm <<= 12;
		uint64 daddr = addr + imm;
		snprintf(instr->text, sizeof(instr->text), "%s x%d, #0x%04x%08x", op ? "adrp" : "adr", Rd, (uint32)(daddr >> 32), (uint32)(daddr & 0xFFFFFFFF));
