...
static int Replace_memcpy() {
	uint32 destPtr = PARAM(0);
	uint32 srcPtr = PARAM(1);
	uint32 bytes = PARAM(2);
	bool skip = false;
	if (!bytes) {
		RETURN(destPtr);
		return 10;
	...
	if (!skip && bytes != 0) {
		...
		if (!dst || !src) {
			...
			const uint32 blocks = bytes & ~0x0f;
			for (uint32 offset = 0; offset < blocks; offset += 0x10) {
				memcpy(dst + offset, src + offset, 0x10);
			}
			for (uint32 offset = blocks; offset < bytes; ++offset) {
...
static int Replace_memcpy_swizzled() {
	...
	if (dstp && srcp) {
		const u8 *ysrcp = srcp;
		for (uint32 y = 0; y < h; y += 8) {
			const u8 *xsrcp = ysrcp;
			for (uint32 x = 0; x < pitch; x += 16) {
				const u8 *src = xsrcp;
				for (int n = 0; n < 8; ++n) {
					memcpy(dstp, src, 16);
					src += pitch;
...
static int Replace_strlen() {
	uint32 srcPtr = PARAM(0);
	const char *src = (const char *)Memory::GetPointer(srcPtr);
	uint32 len = src ? (u32)strlen(src) : 0UL;
	RETURN(len);
...
static int Replace_gta_dl_write_matrix() {
	uint32 *ptr = (uint32 *)Memory::GetPointer(PARAM(0));
	uint32 *src = (u32_le *)Memory::GetPointer(PARAM(2));
	uint32 matrix = PARAM(1) << 24;
	...
	uint32 *dest = (u32_le *)Memory::GetPointer(ptr[0]);
	if (!dest) {
		RETURN(0);
		return 38;
	}
...
static int Replace_dl_write_matrix() {
	uint32 *dlStruct = (uint32 *)Memory::GetPointer(PARAM(0));
	uint32 *src = (uint32 *)Memory::GetPointer(PARAM(2));
	...
	uint32 *dest = (uint32 *)Memory::GetPointer(dlStruct[2]);
	if (!dest) {
		RETURN(0);
		return 60;
	}
...
static bool GetMIPSStaticAddress(uint32 &addr, s32 lui_offset, s32 lw_offset) {
...
static int Hook_godseaterburst_blit_texture() {
	...
	const uint32 fb_info = Memory::Read_U32(fb_infoaddr);
	const uint32 fb_address = Memory::Read_U32(fb_info);
...
static int Hook_starocean_write_stencil() {
	const uint32 fb_address = currentMIPS->r[MIPS_REG_T7];
...
static int Hook_topx_create_saveicon() {
	const uint32 fb_address = currentMIPS->r[MIPS_REG_V0];
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00044000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00044000, currentMIPS->pc);
	}
...
static int Hook_ff1_battle_effect() {
	const uint32 fb_address = currentMIPS->r[MIPS_REG_A1];
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00088000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00088000, currentMIPS->pc);
	}
...
static int Hook_brandish_download_frame() {
	...
	const uint32 fb_index = (Memory::Read_U32(fb_info + fb_index_offset) + 1) & 1;
	const uint32 fb_address = 0x4000000 + (0x44000 * fb_index);
	const uint32 dest_address = currentMIPS->r[MIPS_REG_A1];
...
static int Hook_growlanser_create_saveicon() {
	const uint32 fb_address = Memory::Read_U32(currentMIPS->r[MIPS_REG_SP] + 4);
	const uint32 fmt = Memory::Read_U32(currentMIPS->r[MIPS_REG_SP]);
	const uint32 sz = fmt == GE_FORMAT_8888 ? 0x00088000 : 0x00044000;
...
static int Hook_sd_gundam_g_generation_download_frame() {
	const uint32 fb_address = Memory::Read_U32(currentMIPS->r[MIPS_REG_SP] + 8);
	const uint32 fmt = Memory::Read_U32(currentMIPS->r[MIPS_REG_SP] + 4);
	const uint32 sz = fmt == GE_FORMAT_8888 ? 0x00088000 : 0x00044000;
	if (Memory::IsVRAMAddress(fb_address) && fmt <= 3) {
		gpu->PerformMemoryDownload(fb_address, sz);
		CBreakPoints::ExecMemCheck(fb_address, true, sz, currentMIPS->pc);
...
static int Hook_kirameki_school_life_download_frame() {
	const uint32 fb_address = currentMIPS->r[MIPS_REG_A2];
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00088000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00088000, currentMIPS->pc);
	}
...
static int Hook_orenoimouto_download_frame() {
	const uint32 fb_address = currentMIPS->r[MIPS_REG_A4];
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00088000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00088000, currentMIPS->pc);
	}
...
static int Hook_suikoden1_and_2_download_frame_1() {
	const uint32 fb_address = currentMIPS->r[MIPS_REG_S4];
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00088000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00088000, currentMIPS->pc);
	}
...
static int Hook_suikoden1_and_2_download_frame_2() {
	const uint32 fb_address = currentMIPS->r[MIPS_REG_S2];
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00088000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00088000, currentMIPS->pc);
	}
...
static int Hook_rezel_cross_download_frame() {
	const uint32 fb_address = Memory::Read_U32(currentMIPS->r[MIPS_REG_SP] + 0x1C);
	const uint32 fmt = Memory::Read_U32(currentMIPS->r[MIPS_REG_SP] + 0x14);
	const uint32 sz = fmt == GE_FORMAT_8888 ? 0x00088000 : 0x00044000;
	if (Memory::IsVRAMAddress(fb_address) && fmt <= 3) {
		gpu->PerformMemoryDownload(fb_address, sz);
		CBreakPoints::ExecMemCheck(fb_address, true, sz, currentMIPS->pc);
...
static int Hook_bokunonatsuyasumi4_download_frame() {
	const uint32 fb_address = currentMIPS->r[MIPS_REG_A3];
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00044000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00044000, currentMIPS->pc);
	}
...
static int Hook_danganronpa2_1_download_frame() {
	const uint32 fb_base = currentMIPS->r[MIPS_REG_V0];
	const uint32 fb_offset = currentMIPS->r[MIPS_REG_V1];
	const uint32 fb_offset_fix = fb_offset & 0xFFFFFFFC;
	const uint32 fb_address = fb_base + fb_offset_fix;
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00088000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00088000, currentMIPS->pc);
	}
...
static int Hook_danganronpa1_1_download_frame() {
	const uint32 fb_base = currentMIPS->r[MIPS_REG_A5];
	const uint32 fb_offset = currentMIPS->r[MIPS_REG_V0];
	const uint32 fb_offset_fix = fb_offset & 0xFFFFFFFC;
	const uint32 fb_address = fb_base + fb_offset_fix;
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00088000);
...
static int Hook_danganronpa1_2_download_frame() {
	...
	const uint32 fb_base = currentMIPS->r[reg_num];
	const uint32 fb_offset = currentMIPS->r[MIPS_REG_V0];
	const uint32 fb_offset_fix = fb_offset & 0xFFFFFFFC;
	const uint32 fb_address = fb_base + fb_offset_fix;
	if (Memory::IsVRAMAddress(fb_address)) {
...
static int Hook_rewrite_download_frame() {
	const uint32 fb_address = currentMIPS->r[MIPS_REG_A0];
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00088000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00088000, currentMIPS->pc);
	}
...
static int Hook_motorstorm_download_frame() {
	const uint32 fb_address = Memory::Read_U32(currentMIPS->r[MIPS_REG_A1] + 0x18);
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00088000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00088000, currentMIPS->pc);
	}
...
static int Hook_atvoffroadfurypro_download_frame() {
	const uint32 fb_address = currentMIPS->r[MIPS_REG_S2];
	const uint32 fb_size = (currentMIPS->r[MIPS_REG_S4] >> 3) * currentMIPS->r[MIPS_REG_S3];
	if (Memory::IsVRAMAddress(fb_address)) {
...
static int Hook_atvoffroadfuryblazintrails_download_frame() {
	const uint32 fb_address = currentMIPS->r[MIPS_REG_S5];
	const uint32 fb_size = (currentMIPS->r[MIPS_REG_S3] >> 3) * currentMIPS->r[MIPS_REG_S2];
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, fb_size);
		CBreakPoints::ExecMemCheck(fb_address, true, fb_size, currentMIPS->pc);
	}
...
static int Hook_atvoffroadfuryprodemo_download_frame() {
	const uint32 fb_address = currentMIPS->r[MIPS_REG_S5];
	const uint32 fb_size = ((currentMIPS->r[MIPS_REG_A0] + currentMIPS->r[MIPS_REG_A1]) >> 3) * currentMIPS->r[MIPS_REG_S2];
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, fb_size);
		CBreakPoints::ExecMemCheck(fb_address, true, fb_size, currentMIPS->pc);
	}
...
static int Hook_unendingbloodycall_download_frame() {
	const uint32 fb_address = currentMIPS->r[MIPS_REG_T3];
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00088000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00088000, currentMIPS->pc);
	}
...
static int Hook_katamari_render_check() {
	const uint32 fb_address = Memory::Read_U32(currentMIPS->r[MIPS_REG_A0] + 0x3C);
	const uint32 fbInfoPtr = Memory::Read_U32(currentMIPS->r[MIPS_REG_A0] + 0x40);
	if (Memory::IsVRAMAddress(fb_address) && fbInfoPtr != 0) {
		const uint32 sizeInfoPtr = Memory::Read_U32(fbInfoPtr + 0x0C);
		// These are the values it uses to control the loop.
		// Width in memory appears to be stride / 8.
		const uint32 width = Memory::Read_U16(sizeInfoPtr + 0x08) * 8;
		// Height in memory is also divided by 8 (but this one isn't hardcoded.)
		const uint32 heightBlocks = Memory::Read_U16(sizeInfoPtr + 0x0A);
		// For some reason this is the number of heightBlocks less 1.
		const uint32 heightBlockCount = Memory::Read_U8(fbInfoPtr + 0x08) + 1;

		const uint32 totalBytes = width * heightBlocks * heightBlockCount;
...
static int Hook_mytranwars_upload_frame() {
	uint32 fb_address = currentMIPS->r[MIPS_REG_S0];
	if (Memory::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryUpload(fb_address, 0x00088000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00088000, currentMIPS->pc);
	}
...
static uint32 marvelalliance1_copy_src = 0;
static uint32 marvelalliance1_copy_dst = 0;
static uint32 marvelalliance1_copy_size = 0;
...
static bool WriteReplaceInstruction(uint32 address, int index) {
	uint32 prevInstr = Memory::Read_Instruction(address, false).encoding;
...
void WriteReplaceInstructions(uint32 address, u64 hash, int size) {
	...
	for (int index : indexes) {
		...
		if (entry->flags & REPFLAG_HOOKEXIT) {
			...
			for (uint32 offset = 0; offset < (u32)size; offset += 4) {
				const uint32 op = Memory::Read_Instruction(address + offset, false).encoding;
...
void RestoreReplacedInstruction(uint32 address) {
	const uint32 curInstr = Memory::Read_U32(address);
...
void RestoreReplacedInstructions(uint32 startAddr, uint32 endAddr) {
	...
	for (auto it = start; it != end; ++it) {
		const uint32 addr = it->first;
		const uint32 curInstr = Memory::Read_U32(addr);
		if (MIPS_IS_REPLACEMENT(curInstr)) {
...
bool GetReplacedOpAt(uint32 address, uint32 *op) {
	uint32 instr = Memory::Read_Opcode_JIT(address).encoding;
...
bool CanReplaceJalTo(uint32 dest, const ReplacementTableEntry **entry, uint32 *funcSize) {
