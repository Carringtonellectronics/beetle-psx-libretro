...
void Buffer::Take(size_t length, std::string *dest) {
	if (length > data_.size()) {
		ERROR_LOG(BUFFER,"Truncating length in Buffer::Take()");
		length = data_.size();
	}
	dest->resize(length);
	if (length > 0) {
...
void Buffer::Skip(size_t length) {
	if (length > data_.size()) {
		ERROR_LOG(BUFFER,"Truncating length in Buffer::Skip()");
		length = data_.size();
	}
	data_.erase(data_.begin(), data_.begin() + length);
}
...
void Buffer::Printf(const char *fmt, ...) {
  ...
  if (retval >= (ssize_t)sizeof(buffer)) {
    ...
    ERROR_LOG(BUFFER,"Buffer::Printf truncated output");
  }
  if (retval < 0) {
    ERROR_LOG(BUFFER,"Buffer::Printf failed");
  }
  va_end(vl);
...
bool Buffer::FlushSocket(uintptr_t sock) {
	for (size_t pos = 0, end = data_.size(); pos < end; ) {
		...
		if (sent < 0) {
			ERROR_LOG(BUFFER,"FlushSocket failed");
			return false;
		}
		pos += sent;
...
bool Buffer::ReadAll(int fd, int hintSize) {
	...
	while (true) {
		...
		if (retval == 0) {
			...
			ERROR_LOG(BUFFER,"Error reading from buffer: %i", retval);
			return false;
		}
