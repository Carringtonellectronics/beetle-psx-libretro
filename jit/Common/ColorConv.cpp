...
inline u16 RGBA8888toRGB565(uint32 px) {
	return ((px >> 3) & 0x001F) | ((px >> 5) & 0x07E0) | ((px >> 8) & 0xF800);
}

inline u16 RGBA8888toRGBA4444(uint32 px) {
	return ((px >> 4) & 0x000F) | ((px >> 8) & 0x00F0) | ((px >> 12) & 0x0F00) | ((px >> 16) & 0xF000);
}

inline u16 BGRA8888toRGB565(uint32 px) {
	return ((px >> 19) & 0x001F) | ((px >> 5) & 0x07E0) | ((px << 8) & 0xF800);
}

inline u16 BGRA8888toRGBA4444(uint32 px) {
	return ((px >> 20) & 0x000F) | ((px >> 8) & 0x00F0) | ((px << 4) & 0x0F00) | ((px >> 16) & 0xF000);
}

inline u16 BGRA8888toRGBA5551(uint32 px) {
	return ((px >> 19) & 0x001F) | ((px >> 6) & 0x03E0) | ((px << 7) & 0x7C00) | ((px >> 16) & 0x8000);
}

inline u16 RGBA8888toRGBA5551(uint32 px) {
...
void convert4444_gl(u16* data, u32* out, int width, int l, int u) {
	for (int y = l; y < u; ++y) {
		for (int x = 0; x < width; ++x) {
			uint32 val = data[y*width + x];
			uint32 r = ((val >> 12) & 0xF) * 17;
			uint32 g = ((val >> 8) & 0xF) * 17;
			uint32 b = ((val >> 4) & 0xF) * 17;
			uint32 a = ((val >> 0) & 0xF) * 17;
...
void convert565_gl(u16* data, u32* out, int width, int l, int u) {
	for (int y = l; y < u; ++y) {
		for (int x = 0; x < width; ++x) {
			uint32 val = data[y*width + x];
			uint32 r = Convert5To8((val >> 11) & 0x1F);
			uint32 g = Convert6To8((val >> 5) & 0x3F);
			uint32 b = Convert5To8((val)& 0x1F);
...
void convert5551_gl(u16* data, u32* out, int width, int l, int u) {
	for (int y = l; y < u; ++y) {
		for (int x = 0; x < width; ++x) {
			uint32 val = data[y*width + x];
			uint32 r = Convert5To8((val >> 11) & 0x1F);
			uint32 g = Convert5To8((val >> 6) & 0x1F);
			uint32 b = Convert5To8((val >> 1) & 0x1F);
			uint32 a = (val & 0x1) * 255;
			out[y*width + x] = (a << 24) | (b << 16) | (g << 8) | r;
		}
	}
}
...
void convert4444_dx9(u16* data, u32* out, int width, int l, int u) {
	for (int y = l; y < u; ++y) {
		for (int x = 0; x < width; ++x) {
			uint32 val = data[y*width + x];
			uint32 r = ((val >> 0) & 0xF) * 17;
			uint32 g = ((val >> 4) & 0xF) * 17;
			uint32 b = ((val >> 8) & 0xF) * 17;
			uint32 a = ((val >> 12) & 0xF) * 17;
			out[y*width + x] = (a << 24) | (b << 16) | (g << 8) | r;
		}
	}
}
...
void convert565_dx9(u16* data, u32* out, int width, int l, int u) {
	for (int y = l; y < u; ++y) {
		for (int x = 0; x < width; ++x) {
			uint32 val = data[y*width + x];
			uint32 r = Convert5To8((val)& 0x1F);
			uint32 g = Convert6To8((val >> 5) & 0x3F);
			uint32 b = Convert5To8((val >> 11) & 0x1F);
			out[y*width + x] = (0xFF << 24) | (b << 16) | (g << 8) | r;
		}
	}
}
...
void convert5551_dx9(u16* data, u32* out, int width, int l, int u) {
	for (int y = l; y < u; ++y) {
		for (int x = 0; x < width; ++x) {
			uint32 val = data[y*width + x];
			uint32 r = Convert5To8((val >> 0) & 0x1F);
			uint32 g = Convert5To8((val >> 5) & 0x1F);
			uint32 b = Convert5To8((val >> 10) & 0x1F);
			uint32 a = ((val >> 15) & 0x1) * 255;
			out[y*width + x] = (a << 24) | (b << 16) | (g << 8) | r;
		}
	}
}
...
void ConvertBGRA8888ToRGBA8888(uint32 *dst, const uint32 *src, uint32 numPixels) {
	...
	uint32 sseChunks = numPixels / 4;
	if (((intptr_t)src & 0xF) || ((intptr_t)dst & 0xF)) {
		sseChunks = 0;
	}
	for (uint32 i = 0; i < sseChunks; ++i) {
...
void ConvertRGBA565ToRGBA8888(uint32 *dst32, const u16 *src, uint32 numPixels) {
#ifdef _M_SSE
	...
	uint32 i = sseChunks * 8;
#else
	uint32 i = 0;
#endif

	u8 *dst = (u8 *)dst32;
	for (uint32 x = i; x < numPixels; x++) {
		u16 col = src[x];
...
void ConvertRGBA5551ToRGBA8888(uint32 *dst32, const u16 *src, uint32 numPixels) {
#ifdef _M_SSE
	const __m128i mask5 = _mm_set1_epi16(0x001f);
	const __m128i mask8 = _mm_set1_epi16(0x00ff);
...
void ConvertRGBA4444ToRGBA8888(uint32 *dst32, const u16 *src, uint32 numPixels) {
#ifdef _M_SSE
...
void ConvertABGR565ToRGBA8888(uint32 *dst32, const u16 *src, uint32 numPixels) {
	u8 *dst = (u8 *)dst32;
	for (uint32 x = 0; x < numPixels; x++) {
		u16 col = src[x];
...
void ConvertABGR1555ToRGBA8888(uint32 *dst32, const u16 *src, uint32 numPixels) {
	u8 *dst = (u8 *)dst32;
	for (uint32 x = 0; x < numPixels; x++) {
		u16 col = src[x];
		dst[x * 4] = Convert5To8((col >> 11) & 0x1f);
...
void ConvertABGR4444ToRGBA8888(uint32 *dst32, const u16 *src, uint32 numPixels) {
	u8 *dst = (u8 *)dst32;
	for (uint32 x = 0; x < numPixels; x++) {
		u16 col = src[x];
...
void ConvertRGBA4444ToBGRA8888(uint32 *dst32, const u16 *src, uint32 numPixels) {
	u8 *dst = (u8 *)dst32;
	for (uint32 x = 0; x < numPixels; x++) {
		u16 c = src[x];
		uint32 r = c & 0x000f;
...
void ConvertRGBA5551ToBGRA8888(uint32 *dst, const u16 *src, uint32 numPixels) {
	for (uint32 x = 0; x < numPixels; x++) {
		...
		uint32 r = c & 0x001f;
		uint32 g = (c >> 5) & 0x001f;
		uint32 b = (c >> 10) & 0x001f;
		// We force an arithmetic shift to get the sign bits/
		uint32 a = ((s32)(s16)c) & 0xff000000;
